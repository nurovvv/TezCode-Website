...
#             ]
#         },
# We want to replace the generic "Learn about ... in Machine Learning" or "Learn how ..." with the better text.

updated_content = content
for key, text in ml_content_map.items():
    # The slug format is id with hyphens:
    slug = key.replace('_', '-') + '-basics'
    # Find the section by matching id: 'ml-something-basics' and replacing its content line.
    pattern = r"(id:\s*'" + slug + r"'.*?content:\s*`<p>).*?(</p>`)"
    
    # In case there are multiple lines or different quotes, we can be more flexible
    updated_content = re.sub(pattern, r"\g<1>" + text + r"\g<2>", updated_content, flags=re.DOTALL)
